...
export default function AddressInput({ tokens }) {
    ...
    for (let i = 0; i < tokens.length; i++) {
        ...
        if (tokens[i].content.files) {
            NFTs.push(
                <div key={tokens[i].id} className='flex w-64 flex-col h-max rounded-lg cursor-pointer '>
                            ...
                            <div className='flex flex-row justify-between p-4 h-18 text-md'>
                                <div className="flex flex-col">
                                    <div className='flex flex-row space-x-2'>
                                        <div className=' text-slate-100 font-medium truncate'>{tokens[i].content.metadata.name}</div>
                                        <Image alt="Verified" src="/verified.svg" width="16" height="16"></Image>
                                    </div>
                                    <div className='flex text-orange font-bold text-sm'>{tokens[i].content.metadata.symbol}</div>
                                </div>

                                <div className="flex flex-col items-start h-full">
                                    <a target="_blank" rel="noreferrer" href={"https://www.magiceden.io/item-details/" + tokens[i].id}>
